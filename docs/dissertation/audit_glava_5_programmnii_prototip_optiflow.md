...
> **Обновление от 17.09.2026 — версия пакета 1.7.** Версия обновлена в `optiflow/__init__.py`, `setup.py`, `task_settings.json`, `README.md` (была 1.6 на дату среза 2.09.2026, зафиксированную выше). Бамп приурочен к работам после среза: реализации и тестам штрафа Inv_3 (16.09.2026), предупреждению о структурной невыполнимости Inv_3 при \(D>9N\) (16.09.2026), исправлению регрессии `aco()` — побочного эффекта штрафа Inv_3, а не независимого дефекта (16.09.2026), и первому полному прогону эксперимента \(T_{cpu}(M)/\Delta E\)/`constraint_pass_rate` для \(M\in\{5,10,20,50,100\}\) (17.09.2026, см. врезку в разделе 5.2). Текст самого отчёта (архитектурная декомпозиция, ансамбль метаэвристик, матрицы \(M_{elem}\)) по-прежнему соответствует срезу 2.09.2026 и версии 1.6, кроме мест, явно помеченных обновлениями.
...
**Данные, которых в репозитории нет (состояние на дату среза, 2.09.2026).**  
...
> **Обновление 17.09.2026 — эксперимент проведён, данные получены.** Техническое препятствие (падение `aco()` на больших TEXT/UNSIGNED-пространствах) устранено 16.09.2026 и оказалось побочным эффектом штрафа Inv_3, а не независимым дефектом (см. раздел 3.1). Выполнен полный прогон `scripts/run_dissertation_experiments.py --repeats 10` — 155,6 мин, 600 строк сырых данных (6 точек × 10 повторов × 10 алгоритмов), **0 сбоев**, включая \(M=100\) и заведомо Inv_3-невыполнимую точку. Артефакты: `docs/dissertation/experiments/{raw_runs.csv, summary.md, summary.csv, environment.json}`.
>
> Основные числа (полная таблица mean±std по каждому \((M,N,\text{алгоритм})\) — в `summary.md`):
>
> - **\(T_{cpu}(M)\), сек** (среднее по 10 повторам): при \(M=100\) разброс между алгоритмами — от 2,50 с (`Random`) до **474,1 с** (`Tabu` — единственный метод с выраженно худшим масштабированием: полный перебор соседей \((field, control)\) на каждой итерации); `NSGA-II`/`GA`/`PSO`/`ACO` — от 11,7 до 41,2 с; `BruteForce` завершается мгновенно при \(M\ge 20\) (мгновенный отказ по превышению лимита \(\lvert\Omega\rvert\le 50\,000\), а не полный перебор).
> - **Ground Truth (`brute_force`)** вычислим только при \(M=5\) (\(\lvert\Omega\rvert=16\)) и \(M=10\) (\(\lvert\Omega\rvert=5632\)); при \(M\ge 20\) (\(\lvert\Omega\rvert\) от \(9{,}5\cdot10^5\) до \(\sim\!2{,}2\cdot10^{33}\)) **недоступен** — \(\Delta F\) в этих точках посчитан только относительно \(S_0\), эталон не подменялся лучшим найденным \(F\).
> - **Определение \(S_0\) отсутствует в исходных материалах проекта.** Принято \(S_0=\texttt{space.decode\_layout(space.random\_vector())}\) при фиксированном \(\mathrm{seed}=999999\), отдельном от прогонных seed'ов — **авторское допущение эксперимента, подлежащее подтверждению**, не факт из документации или кода.
> - **`constraint_pass_rate`** (доля прогонов с \(\max_i k_i\le 9\)): \(1{,}0\) у всех алгоритмов при \(M\le 20\) (запас по Inv_3 достаточен); падает у части алгоритмов при \(M=50\) (`NSGA-II` 0,20, `SA` 0,10, `Random` 0,40, `HillClimb` 0,70); резче при \(M=100\) (`NSGA-II`/`Random`/`SA`/`ACO` \(=0\); `HillClimb` 0,30; `GA` 0,80; `PSO`/`Greedy`/`Tabu` держат 1,0); на заведомо Inv_3-невыполнимой точке (\(M=50\), \(N=5\), \(D>9N\)) — **строго 0,0 у всех алгоритмов без исключения**, что ожидаемо и корректно (структурная невыполнимость), и даёт содержательный контраст вместо неинформативных ~100% в feasible-серии.
> - \(S_0\) резко деградирует с ростом \(M\) (мультипликативная модель \(E\) на неоптимизированном разбиении естественно коллапсирует к нулю): raw \(F(S_0)\) от \(+0{,}025\) (\(M=5\)) до \(-0{,}86\) (Inv_3-невыполнимая точка \(M=50\)).
> - Окружение прогона: Python 3.13.7, macOS 26.6.2, Intel Core i9-9980HK @ 2.40GHz, numpy 2.2.6, **однопоточно на одной машине** — \(T_{cpu}\) не претендует на межплатформенную сопоставимость, только на сравнение алгоритмов друг с другом и по \(M\) в идентичных условиях.
>
> Открытый вопрос №1 (эмпирическая проверка сходимости на реалистичном \(D\)) был закрыт ещё 16.09.2026 (`RealisticMillerConvergenceTests`, D=17); данный эксперимент — не повторная проверка того же вопроса, а отдельная задача (T_cpu/ΔE для глав 4–5), ранее заблокированная тем же техническим препятствием.
...
5. Воспроизводимый Ground Truth ограничен \(\lvert\Omega\rvert\le 50\,000\); таблицы \(T_{cpu}(M)\) для \(M\in\{5,\ldots,100\}\) требуют отдельного эксперимента вне текущего репозитория. Эмпирическая проверка сходимости к Inv_3-допустимым решениям на реалистичном \(D\) (порядка 15–20) также требует отдельного эксперимента — см. открытый вопрос №1 во врезке раздела 2.3.  
   **Обновление 17.09.2026:** оба эксперимента проведены. Открытый вопрос №1 закрыт 16.09.2026 (`RealisticMillerConvergenceTests`). Таблицы \(T_{cpu}(M)\)/\(\Delta E\)/`constraint_pass_rate` для \(M\in\{5,10,20,50,100\}\) — `docs/dissertation/experiments/` (сырые данные + сводка), скрипт — `scripts/run_dissertation_experiments.py`; определение \(S_0\) и ограниченность \(T_{cpu}\) одной машиной — авторские допущения эксперимента, см. врезку в разделе 5.2.
