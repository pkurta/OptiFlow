...
def miller_feasibility_warning(
  ...
  return (
    ...
    f"минимизирует превышение ёмкости, но не может свести его к нулю. Отображаемое "
    f"значение F в этом случае может быть показано на нижней границе 0 (реальный, "
    f"нескорректированный скаляр — поле raw_fitness в отчёте/JSON — может быть "
    f"отрицательным)."
  )
