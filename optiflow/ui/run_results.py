...
ALGORITHM_META: Dict[str, Tuple[str, str]] = {
  ...
  "GA": (
    "Классический однокритериальный GA: максимизирует скалярную пригодность F = w₁P + w₂O + w₃R − Penalties.",
...
def _plain_scoring_explanation(*, field_count: int, max_forms: int, weights: CriterionWeights) -> List[str]:
  ...
  return [
    ...
    "  6. Отдельно — жёсткий предел Миллера (Inv_3): если на каком-то экране больше 9 элементов,",
    "     добавляется штраф Penalties (растёт квадратично с превышением). Он не заменяет штраф",
    "     «усталости» из п.4, а действует параллельно с ним.",
    (
      f"  7. Для сравнения алгоритмов всё сворачивается в одно число F = "
      f"{w1}·P + {w2}·O + {w3}·R − Penalties — чем больше F, тем лучше при ваших весах."
    ),
    "",
...
def format_optimization_report(
  ...
  *,
  ...
  lines: List[str] = [
    ...
    (
      f"  Веса свёртки F = w₁P + w₂O + w₃R − Penalties: "
      f"w₁={w1}, w₂={w2}, w₃={w3}"
    ),
  ]
  if fields:
