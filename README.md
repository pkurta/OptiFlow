...
- Вход задачи — нормированные веса \(w_1+w_2+w_3=1\) (пресеты сценариев и связанные слайдеры). P, O, R считаются по сгенерированному интерфейсу; фитнес \(F = w_1 P + w_2 O + w_3 R\).

## Настройка и использование

Функции для контролов редактируются во вкладке «Настройка задачи». Поля, число экранов мастера (N) и веса критериев задаются в «Данные, тип, длина», параметры оптимизации выбираются во вкладке «Алгоритмы», сравнение \(F\) и расчётных P, O, R доступно в «Графики», а экспорт выполняется через меню «Файл → Экспорт веб-страницы».
